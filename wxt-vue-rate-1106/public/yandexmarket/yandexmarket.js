console.log('%cYandexMarket价格转换启动','color:red;font-size:14px;');

let cu_code = 'RUB' //货币代码
let rate = 0   //汇率
let yandexmarket_nodes = [
...
const all=async function(){
    ...
    chrome.storage.local.get(['my_rate','yandexmarket','color'],function(r){
    
            yandexmarket_nodes = r.yandexmarket ||[] //测试时直接读取配置好的节点
            rate = r.my_rate[`rate_${cu_code}`]
            if(r.color){
                color = r.color
            }
            find_node_list(yandexmarket_nodes)
